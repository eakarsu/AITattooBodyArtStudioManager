...
// Portfolio style classification auto-tagging artist work
// Feature: portfolio-style-classification
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AITattooBodyArtStudioManager" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Portfolio style classification auto-tagging artist work.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'portfolio-style-classification', payload, result);
    res.json({ success: true, feature: 'portfolio-style-classification', ...result });
  } catch (err) {
    console.error('portfolio-style-classification analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AITattooBodyArtStudioManager". Return strict JSON only.',
      `Feature: Portfolio style classification auto-tagging artist work. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'portfolio-style-classification', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'portfolio-style-classification']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'portfolio-style-classification', title: 'Portfolio style classification auto-tagging artist work', project: 'AITattooBodyArtStudioManager' });
});
