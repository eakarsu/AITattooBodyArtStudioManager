...
// Healing outcome prediction by artist/style/location
// Feature: healing-outcome-prediction
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Healing outcome prediction by artist/style/location.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'healing-outcome-prediction', payload, result);
    res.json({ success: true, feature: 'healing-outcome-prediction', ...result });
  } catch (err) {
    console.error('healing-outcome-prediction analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Healing outcome prediction by artist/style/location. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'healing-outcome-prediction', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'healing-outcome-prediction']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'healing-outcome-prediction', title: 'Healing outcome prediction by artist/style/location', project: 'AITattooBodyArtStudioManager' });
});
