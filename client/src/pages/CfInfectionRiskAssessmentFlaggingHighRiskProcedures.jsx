...
// Feature: Infection risk assessment flagging high-risk procedures for extra aftercare
...
export default function CfInfectionRiskAssessmentFlaggingHighRiskProcedures() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-infection-risk-assessment-flagging-high-risk-procedures-for-extra/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-infection-risk-assessment-flagging-high-risk-procedures-for-extra', project: 'AITattooBodyArtStudioManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-infection-risk-assessment-flagging-high-risk-procedures-for-extra/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Infection risk assessment flagging high-risk procedures for extra aftercare</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITattooBodyArtStudioManager</p>
      </div>
