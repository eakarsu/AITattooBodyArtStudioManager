...
export default function GapNoAiDrivenPortfolioStyleClassification() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-driven-portfolio-style-classification/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-ai-driven-portfolio-style-classification', project: 'AITattooBodyArtStudioManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-driven-portfolio-style-classification/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No AI-driven portfolio style classification</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AITattooBodyArtStudioManager</p>
      </div>
