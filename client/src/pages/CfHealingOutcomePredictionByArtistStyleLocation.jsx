...
// Feature: Healing outcome prediction by artist/style/location to identify best practices
...
export default function CfHealingOutcomePredictionByArtistStyleLocation() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-healing-outcome-prediction-by-artist-style-location-to/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-healing-outcome-prediction-by-artist-style-location-to', project: 'AITattooBodyArtStudioManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-healing-outcome-prediction-by-artist-style-location-to/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Healing outcome prediction by artist/style/location to identify best practices</h1>
