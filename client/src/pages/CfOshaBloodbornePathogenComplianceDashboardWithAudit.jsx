...
export default function CfOshaBloodbornePathogenComplianceDashboardWithAudit() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-osha-bloodborne-pathogen-compliance-dashboard-with-audit-ready-logs/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-osha-bloodborne-pathogen-compliance-dashboard-with-audit-ready-logs', project: 'AITattooBodyArtStudioManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-osha-bloodborne-pathogen-compliance-dashboard-with-audit-ready-logs/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">OSHA/bloodborne-pathogen compliance dashboard with audit-ready logs</h1>
