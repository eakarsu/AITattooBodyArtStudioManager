...
export default function CfDemandForecastingToOptimizeArtistSchedulingFor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-demand-forecasting-to-optimize-artist-scheduling-for-peak/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-demand-forecasting-to-optimize-artist-scheduling-for-peak', project: 'AITattooBodyArtStudioManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-demand-forecasting-to-optimize-artist-scheduling-for-peak/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Demand forecasting to optimize artist scheduling for peak hours</h1>
