...
export default function CfPortfolioStyleClassificationAutoTaggingArtistWork() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-portfolio-style-classification-auto-tagging-artist-work/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-portfolio-style-classification-auto-tagging-artist-work', project: 'AITattooBodyArtStudioManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-portfolio-style-classification-auto-tagging-artist-work/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Portfolio style classification auto-tagging artist work</h1>
